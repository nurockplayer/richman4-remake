...
Astra Medium 負責研究、選型、拆票、驗收、審查及合併；Luna 預設實作。擁有者不參與例行開發管理。引擎與語言尚未選定，由 Astra 決定。

## 交付目標

macOS／Apple Silicon 第一級支援，最終可直接啟動獨立 `.app`。Runtime 與遊戲內容合理分離；優先可玩性及還原度，不過度泛化。

本 repo 與 Tachiko Fortune／Formosa 獨立。原版素材及衍生素材快取不納入版本控制或自動上傳；預留從擁有者有權使用的本機安裝匯入的路徑，缺少素材時明確列出還原差異。

## 目前狀態

尚在任務／流程 bootstrap，沒有遊戲實作、可執行檔或已驗證的建置指令。第一個可运行切片完成後，應在此提供真實的建置、測試、啟動與素材匯入說明，並移除此暫存狀態。
