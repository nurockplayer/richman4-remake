...
func _set_action_options(player_id: int) -> void:
	...
	if not hospitalized and not SourceShop.enabled(self) and _is_inventory() and _is_graph() and int(tile.get("event_code", -1)) == 15:
...
func is_shop_available() -> bool:
	if SourceShop.enabled(self): return not SourceShop.current(self).is_empty()
...
func _run_ai_turn_dispatch() -> Dictionary:
	...
	while state.get("phase", "") != "game_over" and int(state.get("current_player", -1)) == player_id:
		...
		elif state.get("phase", "") == "await_shop":
