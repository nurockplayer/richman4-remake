...
static func supports_save(data: Dictionary) -> bool:
	...
	return bool(data.get("original_companies", false)) and typeof(version) in [TYPE_INT, TYPE_FLOAT] and int(version) >= 5 and data.get("inventory_supply", {}) is Dictionary and data.get("board_mode", "") == "graph" and data.get("board", []) is Array
...
static func validate(data: Dictionary, supported: bool) -> Array:
	...
	if bool(visit.closed) and data.get("phase") == "await_shop": return ["closed source shop in pending phase"]
