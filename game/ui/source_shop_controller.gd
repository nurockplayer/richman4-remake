...
func sync(owner: Object, blocked: bool) -> void:
	...
	_render(model)
	visible = not blocked
	# Keep the validated visit panel configured beneath another presenter.
	# The controller hides its whole canvas while blocked, and the panel stops
	# receiving pointer input; a dismissed news/fate popup then reveals it on sync.
	panel.visible = true
