...
func run() -> void:
	...
	board.set_map_rotation(0.0, board.size * 0.5)
	board.play_movement([{"player_id": 0, "from": 0, "to": 1, "direction": 2}], 1.0)
	check(board.get_character_frame_direction(0) == 2, "world-right movement keeps the source right-facing frame without rotation")
	board.set_map_rotation(PI / 2.0, board.size * 0.5)
	check(board.get_character_frame_direction(0) == 0, "90-degree rotation maps world-right movement to the source down-facing frame")
	...
	await _run_frames()
	var ordinary_viewport_size: Vector2 = text_board.get_camera_viewport().size
	check(not text_board.texts.has("原版路網") and not text_board.texts.has("原版地圖預覽"), "ordinary player view hides graph debug labels")
	var preview_board := TextBoard.new()
	preview_board.size = Vector2(440, 440)
	root.add_child(preview_board)
	preview_board.set_preview_definition(definition)
	await _run_frames()
	check(preview_board.texts.has("原版地圖預覽"), "preview view retains graph labels")
	check(preview_board.visible_node_indices().size() == definition.board.size(), "preview fits the complete source map in the panel")
	check(preview_board.get_camera_viewport().size.x < ordinary_viewport_size.x, "preview fits the full map instead of retaining the player crop")

	board.queue_free()
	text_board.queue_free()
	preview_board.queue_free()
	await process_frame
