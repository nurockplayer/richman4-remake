...
func _test_machine_doll_cleanup_and_rng() -> void:
	...
	var doll_held_before: int = int(game.state.players[0].tools["機器娃娃"])
	...
	expect(int(game.state.players[0].tools["機器娃娃"]) == doll_held_before - 1, "machine doll consumes one held tool")
