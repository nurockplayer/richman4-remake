...
func _test_machine_doll_cleanup_and_rng() -> void:
	...
	set_turn(game, 0, "await_roll")
	var doll_pool_before: int = int(game.state.inventory_supply.tools["機器娃娃"])
	var doll_held_before: int = int(game.state.players[0].tools.get("機器娃娃", 0))
	...
	expect(int(game.state.inventory_supply.tools["機器娃娃"]) == doll_pool_before + 1, "machine doll returns one to finite supply")
	expect(int(game.state.players[0].tools.get("機器娃娃", 0)) == doll_held_before - 1, "machine doll consumes one held tool")
