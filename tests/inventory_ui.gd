...
func run() -> void:
	...
	expect(int(ui.state.get("version", 0)) == (10 if bool(ui._selected_map_definition.get("supports_original_property_cards", false)) else 9 if bool(ui._selected_map_definition.get("supports_original_hazards", false)) else 8 if bool(ui._selected_map_definition.get("supports_original_statuses", false)) else 7 if bool(ui._selected_map_definition.get("supports_original_companies", false)) else 6 if bool(ui._selected_map_definition.get("original_facilities", false)) else 4), "UI uses facility graph or classic inventory save for selected map")
