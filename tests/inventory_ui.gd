...
func run() -> void:
	...
	expect(int(ui.state.get("version", 0)) == (6 if bool(ui._selected_map_definition.get("original_facilities", false)) else 4), "UI uses facility graph or classic inventory save for selected map")
