...
func run() -> void:
	...
	var before_deposit: int = int(game.state.players[0].deposit)
	var before_bank: Dictionary = game.state.bank.duplicate(true)
	if loan != null and not loan.disabled: loan.pressed.emit()
	check(int(game.state.players[0].loan) == 10000 and int(game.state.players[0].deposit) == before_deposit + 10000 and int(game.state.players[0].cash) == before_cash, "actual source loan button credits deposit without changing cash")
	check(int(game.state.bank.cash) == int(before_bank.cash) and int(game.state.bank.deposits) == int(before_bank.deposits) + 10000 and int(game.state.bank.loans) == int(before_bank.loans) + 10000, "source UI loan preserves bank cash and updates deposit/loan mirrors")
