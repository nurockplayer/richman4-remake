...
func _test_default_setup_and_four_players(ui: Control) -> void:
	_expect(int(ui.state.get("version", 0)) == (7 if bool(ui._selected_map_definition.get("supports_original_companies", false)) else 6 if bool(ui._selected_map_definition.get("original_facilities", false)) else 4), "startup save version matches selected map capability")
...
func _test_actual_setup(ui: Control) -> void:
	...
	_expect(int(snapshot.get("version", 0)) == (7 if bool(ui._selected_map_definition.get("supports_original_companies", false)) else 6 if bool(ui._selected_map_definition.get("original_facilities", false)) else 4), "confirmed setup preserves selected map capability")
