...
func _test_default_setup_and_four_players(ui: Control) -> void:
	_expect(int(ui.state.get("version", 0)) == 5, "startup uses facility setup save version")
...
func _test_actual_setup(ui: Control) -> void:
	...
	_expect(int(snapshot.get("version", 0)) == 5, "confirmed setup uses facility save version five")
...
func _test_missing_system_date() -> void:
	...
	_expect(game != null and int(game.state.get("version", 0)) == 5, "missing clock retains facility setup mode")
	ui.free()
