...
func run() -> void:
	...
	if controller != null and game != null:
		...
		press(viewport, controller.bank_panel.find_child("LoanBorrow", true, false))
		controller.bank_panel.set_amount_text("200")
		controller.bank_panel.confirm_amount()
		await settle()
		check(game.state.players[0].loan == 200, "front calculator submits an ordinary source loan through the host")
		var model: Dictionary = controller.bank_panel.view_model()
		check(model.get("due_date", "") == "1998-04-01", "front displays the core loan due calendar date")
		check(typeof(model.get("special_principal")) == TYPE_INT and int(model.special_principal) == 0 and typeof(model.get("other_deposits")) == TYPE_INT and int(model.other_deposits) == game.state.players[1].deposit, "source summary fields come from the current core account")
		ui._cancel_presentation()
