...
func run() -> void:
	...
	for edition in ["Game", "MultiverseJourney"]:
		...
		var wait_until := Time.get_ticks_msec() + 10000
		while Time.get_ticks_msec() < wait_until:
			await settle()
			panel = ui.source_shell.find_child("SourceShopPanel", true, false)
			if panel != null and panel.has_method("is_open") and panel.call("is_open"): break
...
func normal_shop(definition: Dictionary, options: Dictionary) -> Object:
	for seed_value in range(1,33):
		...
		for step in range(320):
			...
			if game.state.phase == "await_roll": result = game.roll()
