...
func _initialize() -> void:
	...
	for definition in catalog.maps:
		...
		var game:=Game.new_game_on_board(seed_value,4,definition,{"original_facilities":true,"original_gods":true,"original_companies":true,"original_statuses":true,"original_hazards":true,"day_limit":30,"start_date":{"year":1998,"month":1,"day":1}})
		if game==null:
			fail(id+" cannot start hazard game");continue
		map_count+=1
		if game.state.version != 9:
			fail(id+" did not start v9");continue
		...
		print("Hazard source ",id," version=",game.state.version," turns=",turns," phase=",game.state.phase)
	if map_count!=12: fail("Expected twelve hazard-capable source maps")
	print("Hazard source acceptance: ",map_count," maps, ",failures," failures")
