...
from prepare_inventory_assets import (  # noqa: E402
    AssetError as InventoryAssetError,
    _ensure_base_link,
    _merge_image_links,
    _rewrite_base_paths,
    _preflight_write_set,
...
def prepare(zip_path: Path, output: Path, identity_path: Path, base_manifest: Path) -> dict:
    """Decode to a sibling staging tree, preflight, then publish with rollback."""
    zip_path = zip_path.expanduser().resolve()
    identity_path = identity_path.expanduser().resolve()
    base_manifest = base_manifest.expanduser().resolve()
    ...
    if not zip_path.is_file() or not identity_path.is_file() or not base_manifest.is_file():
        raise AssetError("owner ZIP, Panel10 identity, and resolved base manifest must exist")
    for parent in (output, *output.parents):
        if parent.is_symlink():
            raise AssetError(f"output path contains a redirected parent: {parent}")
    if output.resolve(strict=False) in {zip_path, identity_path, base_manifest}:
        raise AssetError("output path aliases a source input")
    output.parent.mkdir(parents=True, exist_ok=True)
    staged = Path(tempfile.mkdtemp(prefix=".source-shop-prepare-", dir=output.parent))
    ...
    try:
        ...
        collect(scene)
        missing = [item for item in referenced if not (staged / item).is_file()]
        ...
        try:
            ...
            for source, target in zip(metadata_sources, metadata_targets):
                os.replace(source, target)
                published.append(target)
            missing = [item for item in referenced if not (output / item).is_file()]
            if missing: raise AssetError(f"published scene manifest has unresolved image paths: {missing[0]}")
            ...
            for target in reversed(published):
                ...
                except OSError as recovery: recovery_errors.append(f"remove {target}: {recovery}")
            for backup, target in reversed(backups):
                try:
                    if target.exists() or target.is_symlink():
                        shutil.rmtree(target) if target.is_dir() and not target.is_symlink() else target.unlink()
                    os.replace(backup, target)
                except OSError as recovery: recovery_errors.append(f"restore {target}: {recovery}")
            if recovery_errors:
                error.add_note(f"rollback incomplete; recovery material retained at {staged}: " + "; ".join(recovery_errors))
...
def main() -> int:
    ...
    except (AssetError, InventoryAssetError, InputError, FormatError, OSError, KeyError, IndexError, zipfile.BadZipFile) as error:
