...
[[ "$(git -C "$TACHIKO_WORKTREE" rev-parse --git-dir 2>/dev/null)" != "" ]] || { echo "PRECONDITION_UNMET: linked Tachiko worktree is not a Git checkout" >&2; exit 2; }
[[ "$(git -C "$TACHIKO_WORKTREE" rev-parse HEAD)" == "$TACHIKO_SHA" ]] || { echo "PRECONDITION_UNMET: linked Tachiko worktree must be $TACHIKO_SHA" >&2; exit 2; }
[[ "$(git -C "$TACHIKO_WORKTREE" rev-parse refs/remotes/origin/main 2>/dev/null || true)" == "$TACHIKO_SHA" ]] || { echo "PRECONDITION_UNMET: linked Tachiko worktree origin/main must be $TACHIKO_SHA" >&2; exit 2; }
[[ -z "$(git -C "$TACHIKO_WORKTREE" status --porcelain)" ]] || { echo "PRECONDITION_UNMET: linked Tachiko worktree is dirty" >&2; exit 2; }
...
cmp "$WORK/base-ids.json" "$WORK/base-persisted-ids.json"
cmp "$WORK/repeat-ids.json" "$WORK/repeat-persisted-ids.json"
...
cmp "$WORK/base-persisted-ids.json" "$WORK/base-roproj-ids.json"
...
cmp "$WORK/base-persisted-ids.json" "$WORK/edited-persisted-ids.json"
cmp "$WORK/base-persisted-ids.json" "$WORK/reopened-persisted-ids.json"
