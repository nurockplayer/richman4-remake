...
const PREFIX: &str = "RICHMAN4_NEWS_EVENTS_ORACLE=";
const COUNT: i64 = 36;
const MAX_SAFE_INTEGER: i64 = 9_007_199_254_740_991;
const MAX_TEXT_BYTES: usize = 4_096;
const SCHEMA_ID: &str = "schema-news-events";
const RUNTIME_FORMAT_VERSION: u32 = 2;
const RUNTIME_DOCUMENT_ID: &str = "richman4-tachiko-news-events-mirror";
const RUNTIME_TITLE: &str = "Richman4 Tachiko news event mirror";
const RUNTIME_SCHEMA_KEY: &str = "news_names";

#[derive(Debug, Deserialize)]
#[serde(deny_unknown_fields)]
struct Candidate {
    news_names: Vec<Event>,
}
...
fn fail(message: impl Into<String>) -> ! {
    eprintln!("tachiko_news_events_mirror: {}", message.into());
    std::process::exit(1)
}
...
fn candidate(bytes: &[u8]) -> Candidate {
    let candidate = serde_json::from_value(parse(bytes))
        .unwrap_or_else(|error| fail(format!("news-event schema rejected: {error}")));
    validate_candidate(&candidate);
    candidate
}

fn validate_candidate(candidate: &Candidate) {
    if candidate.news_names.len() != COUNT as usize {
        fail(format!("expected exactly {COUNT} news event rows"));
    }
    let mut codes = BTreeSet::new();
    for event in &candidate.news_names {
        if !(0..COUNT).contains(&event.news_id) || !codes.insert(event.news_id) {
            fail("news_id must be unique and exactly 0..35");
        }
        if event.display_name.trim().is_empty() {
            fail("display_name must be non-empty");
        }
        if event.display_name.as_bytes().len() > MAX_TEXT_BYTES {
            fail(format!("display_name exceeds {MAX_TEXT_BYTES} bytes"));
        }
    }
    if codes != (0..COUNT).collect::<BTreeSet<_>>() {
        fail("news_id set must be exactly 0..35");
    }
}

fn field_id(key: &str) -> FieldId {
    format!("field-news-event-{key}").into()
}

fn entity_id(news_id: i64) -> EntityId {
    format!("sid-news-event-{news_id:02}").into()
}
...
fn document(candidate: &Candidate) -> Document {
    ...
    let mut rows = candidate.news_names.clone();
    rows.sort_by_key(|event| event.news_id);
    for event in rows {
        let id = entity_id(event.news_id);
        let fields = BTreeMap::from([
            (
                field_id("news_id"),
                Value::Number(number(event.news_id, "news_id")),
            ),
        ...
        entities.insert(
            ...
            Entity {
                id,
                key: format!("news_event_{:02}", event.news_id).into(),
...
fn runtime_number(value: &JsonNumber) -> i64 {
    let number =
        exact_integral(value).unwrap_or_else(|| fail("runtime news_id must be an integer"));
    if !(0..COUNT).contains(&number) {
        fail("runtime news_id outside 0..35");
    }
    number
}
...
fn runtime_text(value: &str) -> String {
    ...
    if value.as_bytes().len() > MAX_TEXT_BYTES {
        fail(format!(
            "runtime display_name exceeds {MAX_TEXT_BYTES} bytes"
        ));
    }
    value.to_owned()
}
...
fn normalize(path: &Path, output: &Path) {
    ...
    {
        fail("runtime metadata does not match M6 document contract");
    }
    let expected: BTreeSet<String> = (0..COUNT)
        .map(|code| format!("news_event_{code:02}"))
        .collect();
    if runtime.entities.keys().cloned().collect::<BTreeSet<_>>() != expected {
        fail("runtime entities must be exactly news_event_00 through news_event_35");
    }
    let mut rows = Vec::new();
    for code in 0..COUNT {
        let key = format!("news_event_{code:02}");
        let entity = &runtime.entities[&key];
        if entity.schema != RUNTIME_SCHEMA_KEY {
            fail(format!("runtime entity {key} schema mismatch"));
        }
        let actual_code = runtime_number(&entity.fields.news_id);
        if actual_code != code {
            fail(format!(
    ...
    write_new(
        output,
        serde_json::to_vec_pretty(&json!({"news_names": rows}))
...
fn identity_document(document: &Document, output: &Path) {
    ...
    let schema = document
        ...
        .unwrap_or_else(|| fail("news_names schema missing from storage"));
    if schema.id.as_str() != SCHEMA_ID {
        fail("news_names schema identity mismatch");
    }
    if schema.fields.len() != 2 {
        fail("news_names schema has unexpected field count");
    }
    let mut field_ids = Map::new();
    for (key, expected_type) in [
        ("news_id", FieldType::Number),
        ...
        let field = schema
            ...
            .unwrap_or_else(|| fail(format!("news event field missing from storage: {key}")));
        if field.field_type != expected_type || !field.required {
            fail(format!(
                "news event field has wrong stored type or required flag: {key}"
            ));
        }
    ...
    let mut rows = Vec::new();
    for code in 0..COUNT {
        let expected_key = format!("news_event_{code:02}");
        let entity = document
            .entities
            .values()
        ...
        let stored_code = match entity.fields.get(&field_id("news_id")) {
            Some(Value::Number(value)) => value.get() as i64,
            _ => fail(format!("{expected_key} has no typed news_id")),
        };
        if stored_code != code || entity.id != entity_id(code) {
            fail(format!(
                "stored news event identity mismatch: {expected_key}"
            ));
        }
        rows.push(json!({"news_id": code, "entity_id": entity.id.to_string()}));
    }
    if document.entities.len() != COUNT as usize {
        fail("stored news event entity count mismatch");
    }
    write_new(
        output,
...
fn check_edit(before_path: &Path, after_path: &Path) {
    ...
    if before.entities.len() != COUNT as usize || after.entities != before.entities {
        ...
        for (id, before_entity) in &before.entities {
            ...
            for (field, before_value) in &before_entity.fields {
                ...
                if id == &target && field == &field_id("display_name") {
                    if before_value != &Value::Text("企業翻倍".into())
                        || after_value != &Value::Text("企業翻倍（M6驗證）".into())
                    {
        ...
        fail("edited document did not contain the isolated M6 change");
    }
}
...
fn reorder(input: &Path, output: &Path) {
    ...
    let rows = root
        ...
        .unwrap_or_else(|| fail("news_names missing"));
    rows.reverse();
    write_new(
        output,
