...
if [[ "${1:-}" == "--m6-inner" ]]; then
  shift
  ORIGINAL_ROOT=${M6_ORIGINAL_ROOT:-}
  WORK=${M6_WORK:-}
  SOURCE_ROOT=${M6_SNAPSHOT_ROOT:-}
  CANDIDATE_HEAD=${M6_CANDIDATE_HEAD:-}
  [[ -n "$ORIGINAL_ROOT" && -n "$WORK" && -n "$SOURCE_ROOT" && -n "$CANDIDATE_HEAD" && -d "$WORK" && -d "$SOURCE_ROOT" ]] || {
    echo "PRECONDITION_UNMET: invalid M6 snapshot handoff" >&2; exit 2
  }
  ...
  [[ -f "$SOURCE_ROOT/tools/tachiko_news_events_mirror/adapter.rs" && -f "$SOURCE_ROOT/tools/tachiko_news_events_mirror/run_m6.sh" ]] || {
    echo "PRECONDITION_UNMET: M6 implementation is absent from candidate HEAD" >&2; exit 2
  }
  TACHIKO_WORKTREE=''
  cleanup() {
  ...
  trap cleanup EXIT
elif [[ -n "${M6_ORIGINAL_ROOT:-}" || -n "${M6_WORK:-}" || -n "${M6_CANDIDATE_HEAD:-}" || -n "${M6_SNAPSHOT_ROOT:-}" ]]; then
  echo "PRECONDITION_UNMET: M6 handoff variables require the private inner sentinel" >&2
  exit 2
else
  ORIGINAL_ROOT=$ROOT
  ...
  git -C "$ORIGINAL_ROOT" ls-files --error-unmatch \
    tools/tachiko_news_events_mirror/adapter.rs \
    tools/tachiko_news_events_mirror/run_m6.sh >/dev/null 2>&1 || {
    echo "PRECONDITION_UNMET: M6 implementation must be present at candidate HEAD" >&2
    exit 2
  }
  CANDIDATE_HEAD=$(git -C "$ORIGINAL_ROOT" rev-parse HEAD) || {
    echo "PRECONDITION_UNMET: unable to resolve candidate HEAD" >&2; exit 2
  }
  WORK=$(mktemp -d "${TMPDIR:-/tmp}/richman4-tachiko-m6.XXXXXX")
...
git -C "$ORIGINAL_ROOT" worktree add --detach "$SOURCE_ROOT" "$CANDIDATE_HEAD" >/dev/null
  ...
  exec env -i \
    ...
    M6_ORIGINAL_ROOT="$ORIGINAL_ROOT" M6_CANDIDATE_HEAD="$CANDIDATE_HEAD" \
    M6_SNAPSHOT_ROOT="$SOURCE_ROOT" M6_WORK="$WORK" \
    "$SOURCE_ROOT/tools/tachiko_news_events_mirror/run_m6.sh" --m6-inner "$@"
fi
...
# M6 admits against this immutable, previously proven Tachiko commit.  A
...
TACHIKO_WORKTREE=$(mktemp -d "${TMPDIR:-/tmp}/richman4-tachiko-m6-tachiko.XXXXXX")
...
CHECKER="${SOURCE_ROOT}/tests/tachiko_news_events_mirror/check_news_events.py"
WITNESS="${SOURCE_ROOT}/tests/tachiko_news_events_mirror/source_oracle.gd"
uv run --no-project --offline python "$CHECKER" --self-test --source "$SOURCE_ROOT/game/core/news_events.gd"
godot_rc=0
"$GODOT_PATH" --headless --path "$SOURCE_ROOT" --script "$WITNESS" >"$WORK/godot.log" 2>&1 || godot_rc=$?
[[ "$godot_rc" == 0 ]] || { echo "FAIL: Godot M6 witness exited $godot_rc" >&2; cat "$WORK/godot.log" >&2; exit 1; }
uv run --no-project --offline python "$CHECKER" --source "$SOURCE_ROOT/game/core/news_events.gd" --godot-log "$WORK/godot.log"
sed -n 's/^RICHMAN4_NEWS_EVENTS_ORACLE=//p' "$WORK/godot.log" >"$WORK/candidate.json"
[[ -s "$WORK/candidate.json" ]] || { echo "FAIL: empty M6 Godot candidate" >&2; exit 1; }

mkdir -p "$WORK/adapter-src/src"
cp "$SOURCE_ROOT/tools/tachiko_news_events_mirror/adapter.rs" "$WORK/adapter-src/src/main.rs"
printf '%s\n' \
  '[package]' \
  'name = "richman4-tachiko-news-events-mirror"' \
  'version = "0.1.0"' \
  'edition = "2024"' \
  '[dependencies]' \
...
ADAPTER="$WORK/adapter-src/target/debug/richman4-tachiko-news-events-mirror"
[[ -x "$CLI" && -x "$ADAPTER" ]] || { echo "FAIL: real CLI or M6 adapter binary missing" >&2; exit 1; }

uv run --no-project --offline python "$CHECKER" --candidate-adapter "$ADAPTER" --tachiko-cli "$CLI"
...
# Sole semantic edit: news id 35 label only, preserving schema and entity identity.
"$CLI" set "$WORK/base.roproj" news_event_35.display_name '企業翻倍（M6驗證）' --output "$WORK/edited.ro"
...
MANIFEST="$WORK/evidence-manifest.txt"
{
  ...
  echo "adapter_source=$(sha256 "$SOURCE_ROOT/tools/tachiko_news_events_mirror/adapter.rs")"
  echo "runner_source=$(sha256 "$SOURCE_ROOT/tools/tachiko_news_events_mirror/run_m6.sh")"
  echo "checker_source=$(sha256 "$CHECKER")"
  echo "witness_source=$(sha256 "$WITNESS")"
  echo "source_news_events=$(sha256 "$SOURCE_ROOT/game/core/news_events.gd")"
  echo "oracle_json=$(sha256 "$SOURCE_ROOT/tests/tachiko_news_events_mirror/oracle.json")"
...
cat "$MANIFEST"
echo "M6_PASS: 36 typed news-event rows, deterministic .roproj roundtrip/reopen, stable news-id identities, isolated news-id-35 label edit, frozen negatives and collisions"
